...
7. 測試只證明這個元件自己的程式碼：class、預設值、prop 的對應與轉傳、子元件的組合方式。Radix 提供的行為（開關、焦點管理、Escape、點外部關閉、aria 屬性）、瀏覽器與 jsdom 的原生語意、Tailwind class 換算出來的數值，由它們自己的測試負責，這裡不重測。例外是每個包裝 primitive 的家族留一條接線測試：把 primitive 換成裸元素後會紅，證明元件真的接上了它。
8. 斷言的東西不能是測試自己放進去的。放一個 child 再斷言它出現在父層裡，換成任何元件都會過。
9. class 字串只在它是本 repo 的決策時才斷言（spec §2、§5 寫到的，例如 D16 的遮罩與陰影），用 jsdom 的 `toHaveClass`。browser 測試只留給需要真實版面才看得出來的行為，例如響應式斷點、格線排列、置中；不量固定 class 換算出來的數值（`min-w-45` 量到 180px 這類）。
10. 新寫或改寫的測試先做一次探針：拿掉它要保護的那一行，確認只有它變紅，再還原。用 `.agents/skills/ui-component-review/scripts/probe.sh`，它自己備份與還原；不要用 `git checkout` 還原，元件可能還沒 commit。
...
4. focus ring 與 invalid ring 一律 2px（`focus-visible:ring-2`、`aria-invalid:ring-2`），所有互動元件相同，包含 Checkbox、RadioGroup、Switch、Slider；Slider thumb 的 hover 與 active 光暈也是 2px。shadcn 上游是 3px，抄進來時要改。
5. 輸入文字或選值的元件（Input、Textarea、SelectTrigger、InputGroup）最小寬度 `min-w-45`（180px）。尺寸固定的控制項（Checkbox、RadioGroup、Switch）與依文字決定寬度的 Button 不套。

## 七、完成後的第一層審查

元件寫完、驗收全綠之後，回報 owner 之前，跑 `.agents/skills/ui-component-review` skill：兩個沒看過實作過程的 subagent 平行審查，一個比對跨元件一致性與子元件、prop、story 有沒有使用情境，一個依第五節逐條判定測試並做探針。有規則依據的 finding 直接修，其餘列給 owner 決定。owner 的決定當場寫回本檔或 spec，下一個元件的審查才會拿它當基準；只留在對話裡的決定，下一輪審查看不到，會被建議改回去。
