...
6. 每個匯出元件至少一個測試檔；story 只在有 variant 值得展示時才寫。兩者的規則見第五節。
...
3. 補 `index.ts` 與 `<PascalCase>.test.tsx`；元件有 variant 時再補 `<PascalCase>.stories.tsx`。
...
只靠本檔與 review 把關的有四處：第一節的檔案佈局、第二節第五條的 `interface` 命名與位置、型別宣告與解構參數的排序（`react/jsx-sort-props` 只看 JSX），以及第五節的測試檔是否存在。介面成員排序要機械化得裝 `eslint-plugin-perfectionist`，owner 決定先不加這個相依（2026-09-03）。檔案佈局若日後漏網次數變多，升級選項是 `eslint-plugin-check-file` 的 `filename-naming-convention` 與 `folder-naming-convention`。

## 五、測試

決策見 spec D11：斷言一律寫在 vitest 測試檔，Storybook 只做展示。

1. 元件測試放 `<PascalCase>.test.tsx`，與元件同資料夾，用 React Testing Library 跑 jsdom。查詢走 role 與可及名稱（`getByRole`），不加 test id。
